/**
 * AutoGPT-Termux CLI — Core Dispatcher
...
const BANNER = `
${c.cyan}${c.bold}╔═══════════════════════════════════════════════════╗
║          AutoGPT-Termux  •  autogptx CLI          ║
║    Run AutoGPT AI Agent on Android — No Root      ║
╚═══════════════════════════════════════════════════╝${c.reset}
`;
...
${clr('bold', 'COMMANDS')}
  ...
  ${clr('cyan', 'uninstall')}     Remove proot Ubuntu container and all AutoGPT data
  ${clr('cyan', 'version')}       Print autogpt-termux version
  ${clr('cyan', 'help')}          Show this help message

${clr('bold', 'QUICK START')}
  1. ${clr('yellow', 'autogptx setup')}        ← first-time install
  2. ${clr('yellow', 'autogptx configure')}    ← enter your API key
  3. ${clr('yellow', 'autogptx start')}        ← launch the agent
  4. ${clr('yellow', 'autogptx logs')}         ← watch live output

${clr('bold', 'WEB DASHBOARD')}
  ${clr('blue', 'http://localhost:8000')}  (starts with autogptx start)
...
function printVersion() {
  try {
    ...
    console.log(`autogpt-termux v${pkg.version}`);
  } catch {
    console.log('autogpt-termux v1.0.0');
  }
}
...
function runScript(scriptFile, extraArgs = []) {
  ...
  if (!existsSync(scriptPath)) {
    ...
    console.error(clr('yellow', '  Try reinstalling: npm install -g autogpt-termux'));
    process.exit(1);
  }
...
function uninstall() {
  console.log(clr('red', '\n⚠  WARNING: This will delete the Ubuntu container and all AutoGPT data.'));
...
export function run(args) {
  ...
  if (!scriptFile) {
    ...
    console.error(`  Run ${clr('cyan', 'autogptx help')} to see available commands.\n`);
    process.exit(1);
  }
