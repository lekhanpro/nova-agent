...
const BANNER = `
${c.cyan}${c.bold}╔══════════════════════════════════════════════════════╗
║           Nova Agent  •  novax CLI  v1.1.0           ║
║    AI assistant with native Android superpowers       ║
╚══════════════════════════════════════════════════════╝${c.reset}
`;
...
${clr('bold', 'COMMANDS')}
  ...
  ${clr('cyan', 'status')}        Show agent config and Termux:API tool status
  ${clr('cyan', 'update')}        Update nova_agent.py to the latest version
  ${clr('cyan', 'logs')}          Tail agent session logs
...
${clr('bold', 'QUICK START')}
  ...
  4. ${clr('yellow', 'novax start')}                          ← interactive chat

${clr('bold', 'ANDROID TOOLS (27)')}
  📷 Camera · 📍 GPS · 💬 SMS (read+send) · 🔋 Battery
  📇 Contacts · 🔔 Notifications · 🔊 TTS · 📋 Clipboard
  📶 WiFi · 📳 Vibrate · 💡 Torch · 📊 Sensors
  🔊 Volume · ☀️  Brightness · 📞 Call Log · 💬 Dialog
  🌐 Browser · 📁 Files (read/write/list) · 🎵 Media
  💻 Device Info · 🖥️  Shell
...
${clr('bold', 'EXAMPLES')}
  ...
  ${clr('dim', 'novax ask "Set volume to 50% and turn on torch"')}
  ${clr('dim', 'novax ask "What sensors does my phone have?"')}

${clr('dim', 'Tip: Start with Gemini (free tier) — novax configure → choose Google')}
`;
